...
| Ref | Implementation path | Technical responsibility in simple terms | Relevant code areas | Confidence | Notes |
| --- | ------------------- | ---------------------------------------- | ------------------- | ---------- | ----- |
...
| Ref | Functional Ref | Implementation Ref | Relation | Meaning in simple terms | Confidence | Evidence |
| --- | -------------- | ------------------ | -------- | ----------------------- | ---------- | -------- |
...
67. Has either right-side view been simplified or reorganized merely to align with the other?
68. Are differences between technical boundaries and source boundaries preserved rather than normalized away?
69. Are there no direct Implementation-to-Code-Organization Mermaid relations?
70. Are any observations comparing the two derived only from their independent Functional mappings?

## Separation

71. Was the External Surface View derived independently rather than copied from the Functional View?
72. Was the Functional View derived independently rather than inferred from any neighboring view?
73. Was the Implementation View derived independently rather than copied from the Functional View or Code Organization View?
74. Was the Code Organization View derived independently rather than translated from Functional or Implementation nodes?
75. Are cross-view explanations confined to their relation layers?
76. Are there no direct External-to-Implementation relations?
77. Are there no direct External-to-Code-Organization relations?
78. Could either right-side view be removed or replaced without redefining the other right-side view?

## Diagram

79. Is the External Surface View on the left?
80. Is the Functional View positioned as the semantic center immediately to its right?
81. Are both Implementation and Code Organization positioned to the right of Functional as parallel views?
82. Has either right-side view accidentally been positioned semantically after the other?
83. Does the External hierarchy grow toward Functional?
84. Do both right-side hierarchies grow toward Functional?
85. Are `*E...`, `*F...`, `*I...`, and `*C...` references visible and consistent?
86. Are hierarchy edges visually distinct from semantic cross-view relation edges?
87. Are semantic cross-view relation edges unlabeled?
88. Does the diagram expose useful many-to-many structures without explanatory text on the edges?
89. Is the diagram still reasonably readable on a normal screen?
90. Does the generated layout-constraint topology match the configured view topology?
91. Is there exactly one presentation-only layout constraint from External to Functional?
92. Is there exactly one presentation-only layout constraint from Functional to Implementation?
93. Is there exactly one presentation-only layout constraint from Functional to Code Organization?
94. Are all `layout...@-->` edges visually invisible and excluded from semantic relation tables and relation consistency checks?
95. Has the diagram avoided forcing the two parallel right-side views into a false semantic sequence?

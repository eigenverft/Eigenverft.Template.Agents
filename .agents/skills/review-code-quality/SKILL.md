...
description: Review main-product source code against repository-provided extension-mapped quality rules. Use only for an explicitly requested code-quality or rule review, return only concrete rule violations, and keep repository state unchanged.
---
...
Use this skill as a read-only dispatcher for main-product source-code rules. It discovers relevant source-code file types, loads the rule files registered for them, and reports only concrete violations. It does not define language-specific coding rules itself, implement corrections, or produce a compliance inventory.
...
- Reviewing one or more programming or scripting languages.

## When Not To Use

- Do not activate this skill merely because code will be created, modified, or refactored.
...
- Discover, search, and read source files only as needed for the requested review.
...
9. Select every rule file mapped to a source-code extension in the requested review scope.
10. Select additional rule files explicitly required by the catalog for mixed-language or cross-cutting reviews.
11. Read every selected rule file completely before evaluating the source code.
12. Do not load rule files for unrelated languages.

If the requested review does not identify target files, use all catalog-registered source-code extensions present in the main product. If its scope later includes another extension, repeat the selection and read every newly applicable rule file completely.

## Missing Or Ambiguous Mappings

- Report a missing mapping only when the requested review explicitly identifies a file as source code and its extension has no catalog entry.
...
1. Evaluate the selected rules against main-product source code within the requested review scope and higher-priority repository guidance.
2. Re-evaluate the selection whenever the set of in-scope extensions changes.
3. Report only source-supported violations that have a concrete recommended correction.
